...
<head>
    ...
    <title>Search Results for "{{ $query }}" - ByteX</title>
...
<body>
    ...
    <nav class="bg-black/30 backdrop-blur-lg px-6 py-4 rounded-xl relative z-50">
        <div class="max-w-7xl mx-auto flex justify-between items-center">
            <div class="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-500 to-purple-600">
                <a href="{{ route('home') }}">ByteX</a>
            </div>
            
            <div class="hidden md:block w-1/3">
                <form action="{{ route('products.search') }}" method="GET">
                    <input type="text" name="query" class="w-full bg-gray-800/70 text-white px-4 py-2 rounded-lg" 
    ...
    <div class="max-w-7xl mx-auto px-6 py-8">
        ...
        @if($products->count() > 0)
            ...
            <!-- Pagination links with query parameter preserved -->
            <div class="mt-8">
                {{ $products->appends(['query' => $query])->links() }}
            </div>
        @else
            <div class="bg-gray-800/50 p-8 rounded-xl border border-gray-700/50 text-center">
                <p>No products found matching "{{ $query }}". Try another search term.</p>
                <a href="{{ route('products.index') }}" class="gradient-btn text-white px-4 py-2 rounded-lg mt-4 inline-block">
                    Browse All Products
                </a>
            </div>
        @endif
